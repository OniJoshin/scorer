...
@section('content')
<h1 class="text-3xl font-bold mb-6 text-purple-700">✏️ Edit Game</h1>
...
<form action="{{ route('games.update', $game) }}" method="POST" class="space-y-6">
    ...
    <div>
        ...
        <textarea name="notes" rows="3" class="mt-1 block w-full rounded border-gray-300 shadow-sm">{{ old('notes', $game->notes) }}</textarea>
    </div>
        ...
        <div class="grid grid-cols-2 gap-4">
            @for($i = 1; $i <= 4; $i++)
                <div>
                    ...
                    <input type="number" name="position_points[{{ $i }}]"
                           value="{{ old('position_points.'.$i, $game->position_points[$i] ?? '') }}" min="0"
                           class="mt-1 block w-full rounded border-gray-300 shadow-sm">
                </div>
            @endfor
        </div>
...
function scoringRuleBuilder() {
    let raw = @json($game->scoring_rules ?? []);
    let initialFields = Array.isArray(raw.fields) ? raw.fields : [];
